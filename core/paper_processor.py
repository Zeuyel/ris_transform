...
# 定义评价标准文件路径
RATING_FILES = {
    'CCF': os.path.join('data', 'ccf_data.json'),
    'FMS': os.path.join('data', 'FMS.json'),
    'AJG': os.path.join('data', 'ajg_2024.json'),
    'ZUFE': os.path.join('data', 'zufe.json')
}
...
def main():
    """命令行入口函数"""
    # 测试用例
    test_file = os.path.join("resources", "scopus.ris")  # 使用 resources 目录下的 scopus.ris 文件
    test_folder = "test_output"
    
    try:
        # 检查文件是否存在
        if not os.path.exists(get_resource_path(test_file)):
            print(f"错误: 未找到 {test_file} 文件")
            return
            
        # 调用处理函数
        output_dir = os.path.join('out_ris', test_folder)
        result = process_ris_file(get_resource_path(test_file), ['top', '1a', 'abs3+', 'fmsB+'], output_dir)
